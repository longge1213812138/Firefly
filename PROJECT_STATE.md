...
1. **下一步**：实机测试连续多轮 Pi 任务（第一轮做游戏，第二轮改游戏），验证任务面板正常显示
2. **前置条件**：无阻塞项
3. **验收方法**：启动 EXE → 输入 `/pi 帮我做个小游戏` → 确认 → 再输入 `/pi 帮我修改这个游戏` → 确认 → 查看任务面板无报错
