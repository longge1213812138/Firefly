...
| 能力 | 状态 | 说明 |
|---|---|---|
| 记忆系统 | ✅ 可用 | 全部对话写入本地 SQLite，重启不丢；支持中文关键词检索（`python main.py --search 关键词`）；回答时自动召回相关往事 |
...
| 电脑操作 | ✅ 可用（受限） | 查时间、列目录、读文件、开程序/网页 **直接执行**；写文件、跑命令等 **必须先语音/命令行的二次确认** |
| 桌宠 / GUI | ⏳ 下一阶段 | MVP 先用命令行交互 |
| 声线 / 性格 | ✅ 已预留 | 人设改 `persona/default.md`；音色改 `config.json` 里的 `voice`，后期换 voicedesign/voiceclone 型号即可克隆声线 |
...
**安全闸口（不可关闭）**：删除、覆盖、外发、付款类操作会强制二次确认；所有操作写入 `data/audit.log`，可随时翻旧账。
...
1. 语音打断（说一半插话，助手立刻闭嘴）
2. 桌宠形象（静态图 → Live2D，四态动画）
3. 独立 GUI 操作台（Key 配置、历史浏览、状态面板）
4. 声线定制（MiMo voicedesign 文字描述音色 / voiceclone 复刻）
5. 全权接管的安全放开（在现有闸口框架下逐步开放高风险操作）
