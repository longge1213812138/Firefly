...
| 控制台 GUI | ✅ 可用     | 双击 `控制台.bat`：六页标签——文字聊天（危险操作弹窗确认，支持 `/pi`）、历史记忆检索、**情感**（三维情绪状态 / 情绪曲线 / 发给 TTS 的风格指令预览与微调）、API Key/模型/音色配置（Key 掩码不回显）、**使用统计**（对话次数/操作频率/分类分布/记忆库信息）、人设编辑器、开机自启、体检与审计日志。关闭窗口自动最小化到系统托盘，右键托盘图标可恢复或退出 |
...
| 脚本           | 作用                                     |
| ------------ | -------------------------------------- |
| `启动助手.bat`   | 先自检（23 项全绿才继续），再进入待命（桌宠随语音模式一起出现）      |
...
│  └─ safety.py         安全闸口 + 审计日志
├─ data/                记忆库与日志（运行时生成）
├─ tests/smoke_test.py  离线自检用例（23 项）
└─ 启动助手.bat         一键启动
```
...
10. ~~程序化情绪模型~~ ✅ 已完成（三维数值情绪 + MiMo 推断 + MiMo-TTS 官方导演模式风格指令 + 控制台「情感」页）
11. 让桌宠形象随情绪联动（现在桌宠只有 4 个动作状态，可接入情绪做表情/颜色变化）
12. 全权接管的安全放开（在硬闸口框架下逐步开放更多操作，如批量整理文件）
13. openWakeWord 自训练「Hi Fairy」（彻底离线唤醒，摆脱 Picovoice Key）
