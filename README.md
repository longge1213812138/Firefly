...
流萤有自己的**情绪状态**，是三个数值：**愉悦度**（-1 低落 ~ +1 开心）、**唤醒度**（0 慵懒疲惫 ~ 1 激动亢奋）、**亲密度**（0 陌生 ~ 1 很熟）。

- 每轮对话后由小米 MiMo 大模型推断情绪（用户难过它就跟着低落——共情，不会反过来乐观）
- 情绪会**平滑变化**、也会**随时间自然平复**回基准，重启不丢
- 最终按 **MiMo-V2.5-TTS 官方方案**翻译成自然语言风格指令（默认导演模式：角色 / 场景 / 指导），交给语音去演绎，所以同一句话在不同情绪下听起来是不一样的
- 控制台 **「情感」页**可以看到三维进度条、情绪变化曲线、将发给 TTS 的完整指令，还能手动微调或重置
- 配置在 `config.json` 的 `emotion` 段：`enabled` 开关、`style_mode`（`director` 导演模式 / `brief` 一句话）、`infer_with_llm`（关掉则只用关键词兜底，省钱但更粗）
...
## 三·五、打包成可独立运行的 exe（可选）

想发给别人、或不想装 Python，可以打包：
...
- `pystray` 的托盘后端、`sounddevice` 的 portaudio 动态库、`tests.smoke_test`（自检要用）都已在打包参数里处理
- 首次打包需要 `pyinstaller`：`.venv\Scripts\python.exe -m pip install pyinstaller`

> ⚠️ `dist\config.json` 里含你的 API Key，**发给别人前请先把 Key 清空**。
